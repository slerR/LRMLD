...
module sum_select_16#(
    ...
    logic [  METRIC_W - 1 : 0] AB_m_00MS_d [0 : 3][0 : 3];
    logic [    CONC_W - 1 : 0] AB_l_00MS_d [0 : 3][0 : 3];
    ...
    // Latency, cycles: 1 + 3 + 1 + 1 + 1 + 1 + 3 = 11
    bitonic_merge_8_8#(
        .M_W(METRIC_W),
        .L_W(CONC_W  )
    )merge12 (
    ...
    always_comb begin
        for(int i = 0; i < 4; i++) begin
            o_metrics[i] <= AB_m_00MS_d[3][i];
            o_labels [i] <= AB_l_00MS_d[3][i];
        end
