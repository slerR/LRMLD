...
// Total Latency, cycles: 13
module sum_select_16_bm_ir#(
    ...
    input  logic                       i_valid,
    // first list
    input  logic [16*METRIC_W - 1 : 0] i_metrics,
    input  logic [ 16*LABEL_W - 1 : 0] i_labels,
    // second list
    input  logic [16*METRIC_W - 1 : 0] i_metrics1,
    input  logic [16*LABEL_W1 - 1 : 0] i_labels1,
    // united list
    ...
    // first three sum_select output
    logic [  METRIC_W - 1 : 0] AB_m_0b   [0 : 2][0 : 3];
    logic [    CONC_W - 1 : 0] AB_l_0b   [0 : 2][0 : 3];
    // second three sum_select output
    logic [  METRIC_W - 1 : 0] AB_m_a0   [0 : 2][0 : 3];
    logic [    CONC_W - 1 : 0] AB_l_a0   [0 : 2][0 : 3];
    ...
    logic [  METRIC_W - 1 : 0] AB_m_0b_r [0 : 2][0 : 3];
    logic [    CONC_W - 1 : 0] AB_l_0b_r [0 : 2][0 : 3];
    
    logic [  METRIC_W - 1 : 0] AB_m_a0_r [0 : 2][0 : 3];
    logic [    CONC_W - 1 : 0] AB_l_a0_r [0 : 2][0 : 3]; 
    
    logic [  METRIC_W - 1 : 0] AB_m_11_r [0 : 3];
    logic [    CONC_W - 1 : 0] AB_l_11_r [0 : 3];
    
    logic [  METRIC_W - 1 : 0] AB_m_0b_d [0 : 2][0 : 3];
    logic [    CONC_W - 1 : 0] AB_l_0b_d [0 : 2][0 : 3];
    
    logic [  METRIC_W - 1 : 0] AB_m_a0_d [0 : 2][0 : 3];
    logic [    CONC_W - 1 : 0] AB_l_a0_d [0 : 2][0 : 3];
    
    logic [  METRIC_W - 1 : 0] AB_m_11_d [0 : 3];
    logic [    CONC_W - 1 : 0] AB_l_11_d [0 : 3];
    
    logic [  METRIC_W - 1 : 0] AB_m_00MS_d [0 : 5][0 : 3];
    logic [    CONC_W - 1 : 0] AB_l_00MS_d [0 : 5][0 : 3];
    
    // min element from each group without A0B0
    logic [  METRIC_W - 1 : 0] AB_m_MS   [0 : 6];
    logic [             2 : 0] ss_idx    [0 : 2];
    // index registration
    logic [             2 : 0] ss_idx_r  [0 : 2];
    
    // best sum_select output
    ...
    logic [  METRIC_W - 1 : 0] AB_m_3    [0 : 3];
    logic [    CONC_W - 1 : 0] AB_l_3    [0 : 3];
    ...
    logic [  METRIC_W - 1 : 0] m_mrg12   [0 : 15];
    logic [    CONC_W - 1 : 0] l_mrg12   [0 : 15];
    
    always_ff @(posedge clk) begin
        valid_d <= {valid_d[11 : 0], i_valid};
    end
    ...
    always_ff @(posedge clk) begin
        for(int i = 0; i < 4; i++) begin
            A0_m[i] <= i_metrics [(16*METRIC_W - 1) - i*4*METRIC_W -: 4*METRIC_W];      
            A0_l[i] <= i_labels  [  (16*LABEL_W - 1) - i*4*LABEL_W -: 4*LABEL_W ]; 
                
            B0_m[i] <= i_metrics1[(16*METRIC_W - 1) - i*4*METRIC_W -: 4*METRIC_W];         
            B0_l[i] <= i_labels1 [(16*LABEL_W1 - 1) - i*4*LABEL_W1 -: 4*LABEL_W1];
        end
    end
    
    // Latency, cycles: 1 + 3 = 4
    generate
        for(genvar b = 1; b < 4; b++) begin : A0Bb
            sum_select #(
                .METRIC_W(METRIC_W), 
                .LABEL_W (LABEL_W ), 
                .LABEL_W1(LABEL_W1)
        ...
        for(genvar a = 1; a < 4; a++) begin : AaB0
            sum_select #(
                .METRIC_W(METRIC_W), 
                .LABEL_W (LABEL_W ), 
                .LABEL_W1(LABEL_W1)
    ...
    always_ff @(posedge clk) begin
        ...
        for(int j = 0; j < 4; j++) begin
            AB_m_00MS_d[0][j] <= AB_m_00[j];
            AB_l_00MS_d[0][j] <= AB_l_00[j];               
        end
    ...
    always_comb begin
        for(int i = 0; i < 3; i++) begin
            ...
            AB_m_MS[i+3] <= AB_m_a0_r[i][0];
        end
         AB_m_MS[6] <= AB_m_11_r[0];
    end
    ...
    always_ff @(posedge clk) begin
        case(ss_idx_r[0])
            ...
            3'b010: begin
                for(int i = 0; i < 4; i++) begin
                    AB_m_1[i] <= AB_m_0b_d[2][i];
                    AB_l_1[i] <= AB_l_0b_d[2][i];
                end
            end 
            3'b011: begin
                for(int i = 0; i < 4; i++) begin
            ...
            3'b101: begin
                for(int i = 0; i < 4; i++) begin
                    AB_m_1[i] <= AB_m_a0_d[2][i];
                    AB_l_1[i] <= AB_l_a0_d[2][i];
                end
            end 
            3'b110: begin
                for(int i = 0; i < 4; i++) begin
        ...
        case(ss_idx_r[1])
            ...
            3'b010: begin
                for(int i = 0; i < 4; i++) begin
                    AB_m_2[i] <= AB_m_0b_d[2][i];
                    AB_l_2[i] <= AB_l_0b_d[2][i];
                end
            end 
            3'b011: begin
                for(int i = 0; i < 4; i++) begin
            ...
            3'b101: begin
                for(int i = 0; i < 4; i++) begin
                    AB_m_2[i] <= AB_m_a0_d[2][i];
                    AB_l_2[i] <= AB_l_a0_d[2][i];
                end
            end 
            3'b110: begin
                for(int i = 0; i < 4; i++) begin
        ...
        case(ss_idx_r[2])
            3'b000: begin
                for(int i = 0; i < 4; i++) begin
                    AB_m_3[i] <= AB_m_0b_d[0][i];
                    AB_l_3[i] <= AB_l_0b_d[0][i];
                end
            end 
            3'b001: begin
                for(int i = 0; i < 4; i++) begin
                    AB_m_3[i] <= AB_m_0b_d[1][i];
                    AB_l_3[i] <= AB_l_0b_d[1][i];
                end
            end 
            3'b010: begin
                for(int i = 0; i < 4; i++) begin
                    AB_m_3[i] <= AB_m_0b_d[2][i];
                    AB_l_3[i] <= AB_l_0b_d[2][i];
                end
            end 
            3'b011: begin
                for(int i = 0; i < 4; i++) begin
                    AB_m_3[i] <= AB_m_a0_d[0][i];
                    AB_l_3[i] <= AB_l_a0_d[0][i];
                end
            end 
            3'b100: begin
                for(int i = 0; i < 4; i++) begin
                    AB_m_3[i] <= AB_m_a0_d[1][i];
                    AB_l_3[i] <= AB_l_a0_d[1][i];
                end
            end 
            3'b101: begin
                for(int i = 0; i < 4; i++) begin
                    AB_m_3[i] <= AB_m_a0_d[2][i];
                    AB_l_3[i] <= AB_l_a0_d[2][i];
                end
            end 
            3'b110: begin
                for(int i = 0; i < 4; i++) begin
                    AB_m_3[i] <= AB_m_11_d[i];
                    AB_l_3[i] <= AB_l_11_d[i];
                end
            end 
        endcase 
    end
    ...
    // Latency, cycles: 1 + 3 + 1 + 1 + 1 + 2 + 4 = 13
    bitonic_merge_8_8_ir#(
        .M_W(METRIC_W),
        .L_W(CONC_W  )
    )merge12 (
        .clk (clk    ),
    ...
    always_comb begin
        for(int i = 0; i < 4; i++) begin
            o_metrics[i] <= AB_m_00MS_d[5][i];
            o_labels [i] <= AB_l_00MS_d[5][i];
        end
        
        for(int i = 4; i < 16; i++) begin
            o_metrics[i] <= m_mrg12[i-4];
            o_labels [i] <= l_mrg12[i-4];
        end
    end
    
    assign o_valid = valid_d[12];
