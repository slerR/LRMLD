...
module bitonic_merge_4_4_ir#(
    ...
    logic [L_W-1:0]      ld_pipe [0:7];
    logic [L_W-1:0]      ld      [0:7];
    logic [M_W-1:0]      i_m1_r  [0:3];
    logic [M_W-1:0]      i_m2_r  [0:3];
    ...
    always_ff @(posedge clk) begin
        for (int k = 0; k < 8; k += 4) begin
            for (int m_idx = 0; m_idx < 2; m_idx++) begin
                m2[k+m_idx]       <= s2_m_min[(k/2)+m_idx]; 
                m2[k+m_idx+2]     <= s2_m_max[(k/2)+m_idx];
                idx_l2[k+m_idx]   <= s2_l_min[(k/2)+m_idx]; 
                idx_l2[k+m_idx+2] <= s2_l_max[(k/2)+m_idx];
            end
        end
    end
