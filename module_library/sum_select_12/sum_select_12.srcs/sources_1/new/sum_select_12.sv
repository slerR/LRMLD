...
// Total Latency, cycles: 9
module sum_select_12#(
    ...
    logic [  METRIC_W - 1 : 0] AB_m_00MS_d [0 : 3];
    logic [    CONC_W - 1 : 0] AB_l_00MS_d [0 : 3];
    ...
    always_ff @(posedge clk) begin
        ...
        for(int j = 0; j < 4; j++) begin
            AB_m_00MS_d[j] <= AB_m_00_r[j]; 
            AB_l_00MS_d[j] <= AB_l_00_r[j];               
        end       
    end
    ...
    // Latency, cycles: 1 + 3 + 1 + 1 + 1 + 1 + 1= 9   
    always_comb begin
        for(int i = 0; i < 4; i++) begin
    ...
    always_comb begin
        for(int i = 0; i < 4; i++) begin
            o_metrics[i] <= AB_m_00MS_d[i];
            o_labels [i] <= AB_l_00MS_d[i];
        end
