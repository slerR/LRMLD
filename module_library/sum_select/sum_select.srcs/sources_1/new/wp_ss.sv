module wp_SS #(
    ...
    logic [METRIC_W-1:0]             ss_metrics [0:3];
    logic [CONC_W-1:0]               ss_labels  [0:3];

    logic [4*METRIC_W-1:0]           out_metrics_r;
    ...
    always_ff @(posedge clk) begin
        if (start) begin
            for (int i = 0; i < 4; i++) begin
                out_metrics_r[(3-i)*METRIC_W +: METRIC_W] <= ss_metrics[i];
                out_labels_r [(3-i)*CONC_W   +: CONC_W  ] <= ss_labels[i];
            end
        end
    end
    ...
    always_comb begin
        o_metric = out_metrics_r[(4*METRIC_W - 1) - cnt_out*METRIC_W -: METRIC_W];
